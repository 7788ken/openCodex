...
- `install status` now also exposes install-slot lifecycle visibility (slot count, current slot, and prune preview), and supports previewing retention variants with `--keep <n>` before `install prune`.
