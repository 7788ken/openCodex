# Task Owners

This file maps the current openCodex TODO items to directly responsible owners.

- `T001-codex-cli-capability-inventory.md` — A Jie
- `T002-wrapper-cli-command-spec.md` — A Feng
- `T003-run-command-wrapper.md` — Xiao Lin
- `T004-review-command-wrapper.md` — Xiao Lin
- `T005-session-and-summary-model.md` — Lao Qin
- `T006-profile-and-policy-layer.md` — Lao Qin
- `T007-doctor-command.md` — Lao Yan
- `T008-gateway-spike.md` — Da Liu
- `T009-auto-run-supervisor.md` — Xiao Lin
- `T010-remote-mobile-bridge.md` — A Feng
- `T011-telegram-im-connector.md` — Xiao Lin
- `T012-telegram-cto-delegation.md` — Xiao Lin
- `T013-host-supervisor-runtime.md` — Lao Qin
- `T014-sandbox-advisor-session-contract.md` — A Jie
- `T015-supervisor-session-and-ui-separation.md` — A Feng
- `T016-cto-conversation-and-research-mode.md` — Xiao Lin

## Review Chain

- Product and scope review — A Feng
- Architecture review — Lao Qin
- Validation review — Lao Yan
- Final technical sign-off — Lao Zhou
