...
- `T021` — partially implemented: installed bridge state, transparent `codex` shim install/repair/status flow, bridge-owned live-session records, external send/inbox control, recent-output snapshots, and the first Telegram attach/status relay for active bridge sessions are in place; the remaining gap is richer bidirectional attach semantics plus remote/mobile routing on the same live-session lane
