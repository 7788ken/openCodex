...
- Phase 3 started:
  ...
  - `opencodex bridge tail` now exposes recent bridge session output from the same live session runtime
...
- Phase 4 started:
  ...
  - generic Telegram status questions now report the active bridge-owned live session when no CTO workflow status is available
